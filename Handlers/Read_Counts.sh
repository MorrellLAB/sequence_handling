...
#   What are the dependencies for Read_Counts?
declare -a Read_Counts_Dependencies=(parallel)

#   Define a function to unzip the FastQC report files, extract
#       total number of sequences and sequence length, calculate
#       read counts, and get rid of unzipped FastQC report files
function getCoverage() {
    ...
    echo -e "${sampleName}\t$(( totalSequence * sequenceLength / target ))" >> "${outDirectory}"/"${project}"_counts.txt # Do math and write to output file
...
function Read_Counts() {
    ...
    local outDirectory="$3"/Read_Counts # The out directory to store our files
