...
declare -a Variant_Recalibrator_Dependencies=(java parallel vcftools)
...
function ParseResources() {
    ...
    arguments=() # Create a string of resource arguments for GATK
...
function Variant_Recalibrator() {
    ...
    (set -x; java -Xmx"${memory}" -jar "${gatk}" \
        ...
        -an ReadPosRankSum \
        -mode SNP \
        -input "${out}/Intermediates/${project}_concat.vcf" \
        -R "${reference}" \
