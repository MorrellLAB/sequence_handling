...
The workflow is intended to be 100% reproducible, provided you have the Config file and the version of `sequence_handling` used. It is also intended to be easy for beginner UNIX users to configure and run independently, given that they read the [wiki](https://github.com/MorrellLAB/sequence_handling/wiki).

  

> **NOTE:** This workflow was designed to use [SLURM](https://slurm.schedmd.com) and run on the [Minnesota Supercomputing Institute](https://www.msi.umn.edu). Heavy modifications will be required if these systems are not used. Before a system transition, the pipeline used the Portable Batch System [PBS](https://www.openpbs.org), and some functionality remains.
...
There are multiple entry points for this workflow. Not all dependencies will apply to each handler. Some general dependencies for the workflow as a whole are also listed here:
...
A guide on installing, setting up, and running `sequence_handling` for the first time can be found on the [Getting Started](https://github.com/MorrellLab/sequence_handling/wiki) page of the wiki.
...
For any handler that utilizes PBS job arrays, there is an optional flag, `-t custom_array_indices`, that you can use to re-run specific job array indices that errored out or were aborted. The `custom_array_indices` is a range of arrays and/or a comma-separated list of specific arrays to run WITHOUT spaces in between. Currently, the `-t` flag must be provided as the 3rd argument on the command line (see example below). If left blank (you do not use the `-t` flag), the DEFAULT runs all samples in your sample list. This is helpful if only some of your job arrays fail and you need to re-run only those.
...
The Genotype_GVCFs handler converts the GVCF files for the entire dataset into VCF files, partitioned by chromosome or chromosome part, using [GATK](https://software.broadinstitute.org/gatk/). Breaking the output into chromosome parts allows the process to be split into a task array, greatly speeding up processing time.
...
The Variant_Recalibrator handler uses the [GATK](https://software.broadinstitute.org/gatk/) and user-provided prior sets of "truth" variants to create a model that attempts to separate true variants from false positives. An unfiltered VCF file with the FILTER field annotated is generated.
...
The GBS_Demultiplexing handler will demultiplex raw GBS reads into split FastQ files using the [FASTX-Toolkit](http://hannonlab.cshl.edu/fastx_toolkit/). Code for GBS_Demultiplexing is already present in `sequence_handling` for collaborative alpha testing, but should not be used, as it's extremely buggy.
...
If you feel like tools or alternative processing techniques are missing from `sequence_handling`, please submit a feature request through the Git issues page.
